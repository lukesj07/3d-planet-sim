...
class CelestialBody:
    def __init__(self, name, radius=1.0, cols=64, rows=64, mass=1.0, rotation_speed=0.08, axis_of_rotation=(0, 1, 0),
                 position=(0, 0, 0), velocity=(0, 0, 0), colors=None, 
                 continent_freq=1.0, detail_freq=6.0, scale=0.08, land_threshold=-0.1, noise_bias=0.15):
        """
        params:
    ...
    def apply_gravity(self, other_body):
        """
        gravitational acceleration exerted on another body.
        returns: numpy array for acc vector.
        """
        G = 1.0  # random units
...
class CloudSystem:
    def __init__(self, radius=1.08, cols=128, rows=128):
        self.radius = radius
        self.cols = cols
        self.rows = rows
    ...
    def generate_cloud_mesh(self):
        sphere = create_sphere(radius=self.radius, cols=self.cols, rows=self.rows, method='latitude')
        vertices = np.array(sphere.get_vertices())
        faces = np.array(sphere.get_faces())
        return vertices, faces

    def update_clouds(self, t, original_cloud_vertices, cloud_colors, cloud_mesh, cloud_freq=1.0, cloud_threshold=0.7, depth_intensity=0.05):
...
def smooth_alpha(deformation, threshold, fade_range=0.15):
